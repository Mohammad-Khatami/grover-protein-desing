...
for i in range (len(inv_d)):
    t=t+inv_d[i]
thrsh=t*4.3*.8      #calculate E_th 
####### H-gates in the initialization!
qc_dist.h(var_qubits)
######################################## Now grover! 
for i in range(iteration):    # number of iterations!
    ene_8(qc_dist,0,1,var_qubits,a_qubits,8,num_q_for_res,num_int,num_dec)## A-->a interaction between res_0 and res_1 put in the a_qubits!
    binnary(qc_dist,inv_d[0],b_qubits,num_int,num_dec)##d_a -->b   the distance recripocal between res_0 and res_1 put in the b_qubits! 
    multiply_same(qc_dist,a_qubits,b_qubits,c_qubits,mp_qubits,mp_res_qubits,        ##A*d_a -->c   the distance recripocal and energy between res_0 and res_1 multiplied and put in c_qubits.
                      a_ext_qubits,b_ext_qubits,o_qubits,num_int,num_dec)
    add(qc_dist,c_qubits,d_qubits,o_qubits,m) ###A*d_a >>d     the value in c_qubits is transfered to d_qubits
    multiply_same_dg(qc_dist,a_qubits,b_qubits,c_qubits,mp_qubits,mp_res_qubits,         ##c_qubits clean
                      a_ext_qubits,b_ext_qubits,o_qubits,num_int,num_dec)
    binnary_dg(qc_dist,inv_d[0],b_qubits,num_int,num_dec) ## b_qubits clean
    ene_8_dg(qc_dist,0,1,var_qubits,a_qubits,8,num_q_for_res,num_int,num_dec) ##a_qubits clean
    #####
    ene_8(qc_dist,0,2,var_qubits,a_qubits,8,num_q_for_res,num_int,num_dec)## B-->a   interaction between res_0 and res_2 put in the a_qubits!
    binnary(qc_dist,inv_d[1],b_qubits,num_int,num_dec)##d_b -->b the distance recripocal between res_0 and res_2 put in the b_qubits!
    multiply_same(qc_dist,a_qubits,b_qubits,c_qubits,mp_qubits,mp_res_qubits, ##B*d_b -->c the distance recripocal and energy between res_0 and res_2 multiplied and put in c_qubits.
                      a_ext_qubits,b_ext_qubits,o_qubits,num_int,num_dec)
    add(qc_dist,c_qubits,d_qubits,o_qubits,m) ###B*d_b + A*d_a>>d  the value is added to the d_qubits (E_a*d_a+E_b*d_b)
    multiply_same_dg(qc_dist,a_qubits,b_qubits,c_qubits,mp_qubits,mp_res_qubits,## c_qubits clean
                      a_ext_qubits,b_ext_qubits,o_qubits,num_int,num_dec)
    binnary_dg(qc_dist,inv_d[1],b_qubits,num_int,num_dec)##b_qubits clean
    ene_8_dg(qc_dist,0,2,var_qubits,a_qubits,8,num_q_for_res,num_int,num_dec) ##a_qubits clean
    ####
    ene_8(qc_dist,1,2,var_qubits,a_qubits,8,num_q_for_res,num_int,num_dec)## C-->a interaction between res_1 and res_2 put in the a_qubits!
    binnary(qc_dist,inv_d[2],b_qubits,num_int,num_dec)##d_c -->b the distance recripocal between res_1 and res_2 put in the b_qubits!
    multiply_same(qc_dist,a_qubits,b_qubits,c_qubits,mp_qubits,mp_res_qubits,##C*d_c -->c the distance recripocal and energy between res_1 and res_2 multiplied and put in c_qubits.
                      a_ext_qubits,b_ext_qubits,o_qubits,num_int,num_dec)
    add(qc_dist,c_qubits,d_qubits,o_qubits,m) ###C*d_c >>d ; C*d_c +(B*d_b + A*d_a)>>d  the value is added to the d_qubits (E_a*d_a+E_b*d_b+E_c*d_c)
    binnary_dg(qc_dist,inv_d[2],b_qubits,num_int,num_dec)##b clean
    ####
    ####E_th is put into b_qubits
    binnary(qc_dist,thrsh,b_qubits,num_int,num_dec)
    add(qc_dist,d_qubits,b_qubits,o_qubits,m)  # the E_thotal in d_qubits is subtracted from the E_th in the b_qubits!
    ###
    qc_dist.x(o_qubits[0])
    ###calcullator; now checks if qubits are negative or not
    qc_dist.cz(b_qubits[m-1],o_qubits[0])
    ############ clear out the Oracle!
    qc_dist.x(o_qubits[0])
